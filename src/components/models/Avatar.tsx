...
export function Avatar(props) {
  const group = useRef();
  const { materials } = useGLTF("/models/avatar.glb");
  const { nodes } = useGLTF("/models/blank.glb");
  const { animations: fallingAnimation } = useFBX("/models/animation.fbx");
  const { animations: wavingAnimation } = useFBX("/models/wave.fbx");
  const { animations: dancingAnimation } = useFBX("/models/dance.fbx");
